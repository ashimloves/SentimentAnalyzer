...
public class SentimentExampleIterator implements DataSetIterator {
     ...
     */
    SentimentExampleIterator(String dataDirectory, @org.jetbrains.annotations.NotNull WordVectors wordVectors, int batchSize, int truncateLength, boolean train) throws IOException {
    ...
    private DataSet nextDataSet(int num) throws IOException {
        ...
        for( int i=0; i<num && cursor<totalExamples(); i++ ){
            if(cursor % 2 == 0){
                ...
                String review = FileUtils.readFileToString(positiveFiles[posReviewNumber], CHARSET);
                reviews.add(review);
                positive[i] = true;
            } else {
                //Load negative review
                int negReviewNumber = cursor / 2;
                String review = FileUtils.readFileToString(negativeFiles[negReviewNumber], CHARSET);
                reviews.add(review);
                positive[i] = false;
            }
            cursor++;
    ...
    public INDArray loadFeaturesFromFile(File file, int maxLength) throws IOException {
        String review = FileUtils.readFileToString(file, CHARSET);
     ...
     */
    INDArray loadFeaturesFromString(String reviewContents, int maxLength){
